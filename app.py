...
mais necessite un cycle de vente plus court avec acces direct au DG.
        ...
        st.download_button("Telecharger le rapport complet (.xlsx)",
            ...
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True)/* ── Base : thème clair forcé ────────────────────────────────── */
html, body,
[data-testid="stAppViewContainer"],
[data-testid="stMain"],
.main, .block-container {
...
NAF_QUERIES = {
    "4771Z":"habillement mode vetements boutique",
    "4772A":"chaussures boutique magasin",
    "4772B":"maroquinerie bagages boutique",
    "4777Z":"bijouterie horlogerie boutique",
    "4775Z":"parfumerie cosmetiques beaute",
    "4711B":"supermarche grande distribution",
    "4711C":"hypermarche grande surface",
    "4711A":"superette epicerie alimentaire",
    "4719A":"grand magasin commerce",
    "4719B":"commerce detail specialise",
    "4721Z":"primeur fruits legumes",
    "4741Z":"informatique telephonie boutique",
    "4759A":"meubles ameublement decoration",
    "4778A":"optique lunettes boutique",
    "4778C":"commerce specialise boutique",
    "6420Z":"holding retail commerce",
}
        ...
        for page in range(1, 10):  # Jusqu'à 10 pages par NAF
            try:
                ...
                if resp.status_code == 429:
                    ...
                    resp = requests.get(url, params=params, headers=HEADERS, timeout=12)
                if resp.status_code != 200:
                    errors.append("NAF {}: HTTP {}".format(naf, resp.status_code)); break
                data  = resp.json()
                items = data.get("results", [])
                if not items: break
